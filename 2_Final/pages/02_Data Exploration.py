...
with bike_tab:
    ...
    with st.expander("🚴 Raw Data", expanded=True):
        ...
        st.write(f"The raw bike dataset contains {bike_0.shape[0]:,} rows and {bike_0.shape[1]} columns, with bot categorical and numerical features.")

        st.write(bike_0.dtypes.apply(lambda x: x.name).to_frame('Type').T.style.set_table_styles(
...
with weather_tab:
    ...
    if "weather_data_raw" in st.session_state:
        ...
        with st.expander("🌤️ Raw Dataset", expanded=True):
            ...
            st.markdown(f"The raw weather dataset contains **{weather_0.shape[0]:,}** rows, **{weather_0.shape[1]}** columns, and all numerical features.")

            st.write(weather_0.dtypes.apply(lambda x: x.name).to_frame('Type').T.style.set_table_styles(
                    [{'selector': 'th', 'props': [('text-align', 'left')]},
                    {'selector': 'td', 'props': [('text-align', 'left')]}]
